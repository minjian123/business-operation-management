...
- 入口文档：`README.md`（导航）、`文档/文档首页.md`（全量导航）、`文档/规划/项目规划说明.md`（biz 企业运营管理规划）。
...
- bms 基座有变更（提交信息注明「基座变更，产品仓库需同步」）后，在 bms 仓库根目录运行同步脚本（先 dry-run 审阅，`--apply` 才写入；见 bms `scripts/tools/base-sync/README.md`）：
  `python3 scripts/tools/base-sync/base-sync.py sync --target <biz仓库路径> --apply`
- 本仓库产品专属文件（`文档/规划/项目规划说明.md`、`文档/设计/`、后续业务扩展代码与资料）不受同步影响，不删不覆盖；`文档/用户文档/` 凭据模板不参与同步。
