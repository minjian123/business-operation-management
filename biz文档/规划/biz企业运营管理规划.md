...
- **业务基座定位**：biz 同时作为 BMS 之上「业务运营基座」——向下复用平台通用能力，向上为后继商业运营项目（生产制造、人事等）预留业务模型扩展位；前期设计中不将基础模型焊死，避免后继项目重改。
...
- **现状基线**：规划（含业务域全景分层）已确立；概要设计采购/收付款已迁入；平台扩展代码机制（R4）未落地前，业务先以文档与设计沉淀，随 bms 开发节奏接入。
...
| 财务与资金 | 现金/银行资金与渠道 | （pay 深化） | 银企直连、资金计划、银行对账（渠道层预留） | P2 候选 |
...
| 财务与资金 | 总账/凭证/税务核算 | gl / gl_ | 复式记账与税务核算，深度核算由专业财务软件承接做宽做深 | P3 候选暂缓 |
| 资产与行政 | 固定资产/低值易耗 | fa / fa_ | 资产台账、领用/归还、报废；含办公用品领用 | P2 候选 |
...
biz 承接与预留的业务域明细（与第 5 节标识符登记一一对应；「P3 候选暂缓」「暂缓预留」仅登记与文档沉淀，不进入近期实施）：
...
| 固定资产 | fa / fa_ | 资产台账、领用/归还、报废处置；含办公用品领用 | 扩展候选 |
...
**规划预留**（扩展候选与暂缓域，段位自 15xxxx 顺延；接入开发时按平台「扩展接入流程」在 sys_module 正式注册确认，注册即定；未注册前不建表）：
...
> 应收应付、现金/银行渠道为 pay 模块深化、不单列域，沿用 pay 段位（11xxxx）。

## 6. 演进路线 <a id="roadmap"></a>

| 阶段 | 内容 | 状态 |
| --- | --- | --- |
| 一 立项与骨架 | 本规划（含业务域全景分层）+ 仓库骨架 + 基座软链引用（规范/资料/资源权威源 bms） | 本文档 |
| 二 业务设计 | 运营体系设计总纲 + 概要设计（采购/收付款已迁入；其余已注册模块与高优候选随排期补建），对照平台扩展接入流程 | 部分完成 |
| 三 接入开发 | 平台扩展机制（R4）落地后，五模块 MVP 按 `平台扩展接入流程` 接入平台、随平台阶段交付 | 待定 |
| 四 扩展候选推进 | 按优先级分批：P1（商品主数据/合同/发票/应收应付深化）→ P2（报销/预算/物流/售后/质检/固定资产/CRM 深化）→ P3 暂缓域（总账/市场线索/项目） | 待定 |
| 五 暂缓预研 | 生产制造（物料/BOM、工单）、人事 HR、总账核算：前期模型预留与后继项目承接设计（不实施） | 预留 |
...
| 暂缓域预留不足（生产制造/HR/总账） | 中 | 商品/物料、员工、凭证等基础模型在前期设计即预留扩展位、不焊死；候选域模型与接口保持中立，便于后继项目承接 |
| 候选域范围蔓延 | 中 | 全景分层按 P0~P3 标记优先级，候选域仅文档沉淀与标识符预留，实施严格按优先级与阶段排期，超范围走变更确认 |
| 财务核算深度失控（总账 gl） | 中 | gl 仅承接简易凭证数据，复式记账/成本核算/税务申报深度由专业财务软件承接，biz 不做核算引擎 |
