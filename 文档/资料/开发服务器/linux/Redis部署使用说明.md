...
mjbk 上的 Redis（容器 `bms-redis`）承载本项目的缓存、token 黑名单、分布式锁、限流计数等，
是开发依赖服务之一（平台《开发部署规划》4.3 base 组）。
...
> 开发环境未设密码（内网防火墙隔离）。本项目的 Redis key 遵循《[命名规范](../../../规范/命名规范.md)》：`bms:{租户|global}:{域}:{业务键}`。
