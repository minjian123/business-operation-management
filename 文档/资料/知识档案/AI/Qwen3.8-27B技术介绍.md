...
- **LLM 适配层候选**：Qwen 系是本项目  LLM 适配层的模型候选之一（OpenAI 兼容接口），私有化部署路径（vLLM/Ollama 自托管）可直接加载本模型，见《[LLM 适配层技术介绍](../后端核心/LLM适配层技术介绍.md)》。
...
- **LLM 适配层候选**：Qwen 系是本项目  LLM 适配层的模型候选（外部 API 或 vLLM/Ollama 自托管，均 OpenAI 兼容），本模型可作私有化端点（见《[LLM 适配层技术介绍](../后端核心/LLM适配层技术介绍.md)》）。
- **OCR 复杂场景切换**：本项目阶段十五 OCR 默认 PaddleOCR，复杂版面/公式可切换多模态模型（平台《项目规划说明》3.1 节），本模型是该路径的本地引擎候选（见《[PaddleOCR 技术介绍](../后端核心/PaddleOCR技术介绍.md)》选型对比）。
- **边界**：本项目业务运行时不依赖本模型；它服务本地 AI 工具链（助手多模态能力与模型交叉检查），与业务 AI 链路分离。
