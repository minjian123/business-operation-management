...
> Socket.IO 协议浏览器客户端 · 本项目待办/通知实时到达
...
- **定位**：本项目前端实时推送客户端——待办/通知实时到达、待办角标实时刷新、会话失效即时生效（见平台《架构设计 · 总体架构》6.2 节）。
...
| 房间（Room） | 服务端把连接加入指定房间，向房间广播即只发给相关用户；本项目按用户/租户定向推送 |
| 握手鉴权 | 连接时经 `auth` 选项携带 token，后端在握手阶段校验会话（本项目校验 access token） |
| 连接状态事件 | `connect`/`disconnect`/`connect_error`，用于 UI 状态提示与重连后状态恢复 |
| Sticky Session | 连接升级后必须固定在同一后端实例：nginx 用 ip_hash / Cookie 亲和（见平台《项目规划说明》19.2 节） |
| Redis 适配器 | 后端多实例经 Redis 适配器跨实例广播，客户端无感知（见《[python-socketio 技术介绍](../后端核心/python-socketio技术介绍.md)》） |

## 3. 在本项目中的用途 <a id="usage"></a>

- **待办/通知实时到达**：审批待办提醒、通知中心消息实时推送，替代轮询（见平台《项目规划说明》5 节通知中心）。
- **待办角标实时刷新**：顶栏/侧边栏待办角标作为全局元素实时展示，新待办到达即更新（见平台《项目规划说明》5 节首页工作台）。
- **会话失效即时生效**：管理端强制踢出会话时，经 Socket.IO 广播「会话失效」事件，客户端立即关闭实时连接并跳转登录（见平台《项目规划说明》12 节）。
- **与后端配合**：后端用 python-socketio（Redis 适配器跨实例广播），前端用 socket.io-client，协议两端对齐（见平台《架构设计 · 总体架构》6.1 节）。
- **重连与降级**：断线自动重连、WebSocket 被拦自动降级轮询；重连成功后重新拉取最新待办/通知，避免断线期间漏推（见平台《项目规划说明》3.1 节）。
- **验收口径**：MVP 验收要求「待办提醒经 Socket.IO 实时触达」（见平台《项目规划说明》20 节）。
...
| 候选技术 | 优缺点 | 结论 |
| --- | --- | --- |
| **socket.io-client（选中）** | 自动重连 + 传输降级 + 房间 + 命名事件，与后端 python-socketio 天然配套，MIT | 与本项目「实时推送 + 多实例广播 + 免登场景」诉求完全匹配 |
...
| 文档 | 说明 |
| --- | --- |
| 平台《架构设计 · 总体架构》6.2 节 | 前端技术栈（socket.io-client 条目） |
| 平台《项目规划说明》3.2 节 | 选型理由：待办/通知实时到达 |
| 平台《项目规划说明》19.2 节 | nginx WebSocket 放行与 sticky session 要求 |
