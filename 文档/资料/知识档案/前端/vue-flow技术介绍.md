...
- **定位**：本项目大屏设计器的自由画布——节点自由拖拽（绝对定位）、缩放平移，配合 ECharts 渲染图表节点（见平台《架构设计 · 总体架构》6.2 节）。
...
| 概念 | 说明 |
| --- | --- |
| Node（节点） | 画布上的单元，按绝对坐标（x/y）定位；节点内容可以是自定义 Vue 组件，本项目把 ECharts 图表放进节点 |
...
| Custom Node（自定义节点） | 节点内容即 Vue 组件：可渲染 ECharts、文本、图片等任意内容，本项目图表节点的基础 |
...
- **大屏设计器**：大屏自由画布——节点自由拖拽（绝对定位）、缩放平移，与 gridstack 网格布局分工明确，互不混用（见平台《项目规划说明》3.2 节）。
...
- **三库分工**：列表排序用 vuedraggable、网格布局用 gridstack、自由画布用 vue-flow（见平台《项目规划说明》3.2 节）。
...
| 候选技术 | 优缺点 | 结论 |
| --- | --- | --- |
| **vue-flow（选中）** | Vue 3 官方实现、自由画布 + 缩放平移 + 自定义节点、插件齐全、活跃维护、MIT | 与本项目大屏「自由定位 + 图表节点」诉求完全匹配 |
...
| 文档 | 说明 |
| --- | --- |
| 平台《架构设计 · 总体架构》6.2 节 | 前端技术栈（vue-flow 条目） |
| 平台《项目规划说明》3.2 节 | 选型理由：大屏自由画布，与 gridstack 分工明确 |
