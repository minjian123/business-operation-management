...
在 BMS 中承担**所有业务表主键**的生成。

- **定位**：BMS 分布式全局唯一 ID 生成（业务表主键）。
...
- 分布式全局唯一 ID 生成：**所有业务表统一用雪花 ID 作主键**，关联表用联合主键（见《[项目规划说明](../../../规划/项目规划说明.md#stack-backend)》2.1 节、11.1 节「数据规范」）。
- 分布式环境下无需中心协调即可生成全局唯一 ID，契合 BMS 多实例无状态部署（见《[项目规划说明](../../../规划/项目规划说明.md#deploy-cluster)》19.2 节）。
- **时钟回拨处理配置**：部署时需关注 NTP 时间同步与回拨容忍配置，避免极端情况下 ID 冲突（见《[项目规划说明](../../../规划/项目规划说明.md#stack-backend)》yitter 条目）。
- 与 UUID 相比：雪花 ID 是 64 位整数、**有序可排序**、利于数据库索引顺序写入与范围查询；UUID 无序、占 128 位，索引效率与存储都更差。
- 主键在应用侧生成后随 INSERT 写入，数据库不自增，跨 MySQL/PostgreSQL/达梦三库行为一致（见《[项目规划说明](../../../规划/项目规划说明.md#dbcompat)》11 节）。
...
| 文档 | 说明 |
| --- | --- |
| 《[项目规划说明](../../../规划/项目规划说明.md#stack-backend)》2.1 节 | 后端技术栈：ID 生成（yitter/IdGenerator）条目 |
| 《[项目规划说明](../../../规划/项目规划说明.md#dbrule)》11.1 节 | 数据规范：业务表统一雪花 ID 主键 |
| 《[项目规划说明](../../../规划/项目规划说明.md#tables)》6 节 | 核心数据表清单（主键约定） |
