...
- **定位**：本项目的服务端模板渲染——邮件/通知模板（在线编辑）、简单 HTML 页面输出（导出预览）。
...
| 自动转义（Autoescape） | 默认把输出值做 HTML 转义，防 XSS；本项目渲染 HTML 类模板时务必开启 |
...
| 语法校验 | `env.parse()` 编译模板，可在保存前检查语法错误，本项目邮件模板保存前用它校验 |

## 3. 在本项目中的用途 <a id="usage"></a>

Jinja2 在本项目里负责**服务端模板渲染**，把「模板 + 数据」变成最终的邮件正文、通知文案或简单 HTML 页面。
注意边界：本项目页面主体仍是前后端分离（Vue 3），Jinja2 只用于邮件/通知这类「服务端生成内容」和少量简单 HTML 输出。

- **邮件/通知模板**：模板入库 `sys_mail_template`，页面在线编辑；保存前做**语法校验**（`env.parse()`）与**沙箱渲染测试**（用样例数据试渲染，确认不报错、不越权）（见平台《项目规划说明》3.1 节、5 节「邮件通知」）。
- **多语言内容**：邮件主题/内容的多语言走 `sys_mail_template_i18n` 附表，运行时按收件人 `locale` 渲染（见平台《项目规划说明》11.1 节、《[国际化规范](../../../规范/国际化规范.md)》）。
...
| Django Template | 安全、简单；但**绑定 Django 框架**，本项目用 FastAPI 不引入 Django | 框架耦合，不契合 |
...
- **别拿它渲染主页面**：本项目页面主体是前后端分离（Vue 3），Jinja2 只用于邮件/通知与少量简单 HTML 输出，别用它做 SPA 页面。
...
| 文档 | 说明 |
| --- | --- |
| 平台《项目规划说明》3.1 节 | 选型说明：Jinja2 条目（邮件模板入库、在线编辑、沙箱） |
| 平台《项目规划说明》5 节 | 功能模块：邮件通知（模板入库、语法校验、i18n） |
| 平台《项目规划说明》6 节 | 核心数据表：`sys_mail_template`、`sys_mail_template_i18n` |
