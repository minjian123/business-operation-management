...
- **定位**：本项目唯一的数据库抽象层，所有数据访问（含多数据源、多租户路由）经它完成。
...
- 统一 ORM/Core 两套 API：常规 CRUD 用 ORM 模型，批量与复杂查询用 Core，一套学习曲线覆盖全场景（见平台《项目规划说明》3.1 节）。
- **多数据源**：多个异步 engine 管理，repositories 层按模块指定数据源；主从读写分离、分片路由均落在数据访问层（见平台《项目规划说明》10 节）。
- **租户独立库动态路由**：一租户一库，请求按 tenant 路由到对应异步 engine——懒加载创建 + 闲置回收 + 池上限控制，避免全量租户连接常驻（见平台《项目规划说明》10 节与平台《项目规划说明》14 节）。
- 异步会话生命周期由 FastAPI 依赖注入管理：每个请求创建独立 AsyncSession，事务随请求关闭（见《[FastAPI 技术介绍](FastAPI技术介绍.md)》）。
- **四库兼容**：ORM 层禁用方言特有功能，JSON 等类型用跨方言通用类型，分页统一 limit/offset；软删除唯一键统一建 (唯一字段, deleted_at) 复合索引（见平台《项目规划说明》11 节）。
- 连接池按 worker 规划：总连接数 = worker × (pool_size + max_overflow) ≤ 数据库 max_connections 的 70%（见平台《项目规划说明》14 节）。
...
| Django ORM | 最成熟全家桶；异步支持有限、脱离 Django 框架使用别扭 | 本项目用 FastAPI 而非 Django，不匹配 |
...
- **大事务显式声明边界**：事务内不要混入外部 I/O（HTTP 调用、文件写入），长时间持锁会拖垮数据库；跨服务流程把事务范围缩到最小（见平台《项目规划说明》3.1 节）。
...
- **方言禁用清单**：JSON/分页等用通用类型与通用写法，避免四库迁移时爆差异（见平台《项目规划说明》11 节）。
...
| 文档 | 说明 |
| --- | --- |
| 平台《架构设计 · 总体架构》6.1 节 | 技术栈：数据库抽象条目（异步 ORM/Core，多数据源） |
| 平台《项目规划说明》10 节 | 多数据源策略：多 bind、租户路由、懒加载/回收/池上限 |
| 平台《项目规划说明》11 节 | 数据库兼容：禁用方言功能、跨方言类型约定 |
