...
- **定位**：本项目审批流程引擎——解析前端 bpmn-js 导出的 BPMN XML，驱动审批流转。
...
| UserTask（用户任务） | 需要「人」来处理的节点，本项目用它表示审批节点，等待审批人操作 |
...
| 序列化（Serialization） | 把流程实例当前状态序列化（存/取），本项目用它把流程状态落库、跨请求恢复 |
| 脚本引擎 | 支持在流程节点里执行 Python 脚本，灵活但需注意沙箱安全 |
| 不持久化 | 引擎本身**不内置持久化**：流程实例、任务、审批记录的存储由本项目自建表维护 |

## 3. 在本项目中的用途 <a id="usage"></a>

SpiffWorkflow 在本项目里是**审批流程的执行引擎**。
整体分工：前端 `bpmn-js` 负责拖拽绘制流程定义并导出 BPMN XML，
SpiffWorkflow 负责解析这段 XML 并按图执行审批流转。

- **解析执行 BPMN XML**：支持用户任务（审批节点）、网关（分支/并行）、结束事件等标准元素（见平台《项目规划说明》3.1 节）。
- **同步执行 + 线程池**：SpiffWorkflow 是**同步**库，在 FastAPI 异步栈中放入线程池运行（`run_in_executor`），避免阻塞事件循环（见《[FastAPI 技术介绍](FastAPI技术介绍.md)》）。
- **状态由本项目自建表维护**：引擎本身不持久化，流程实例、任务、审批记录由本项目自建表（如 `wf_process`、流程实例表、审批记录表）存储，经序列化/反序列化跨请求恢复（见平台《项目规划说明》6 节）。
- **流程建模闭环**：管理端用 bpmn-js 拖拽绘制 → 导出 BPMN XML → SpiffWorkflow 执行；流程定义发布新版本、实例启动、节点通过、驳回、结束等经事件总线发布（见平台《项目规划说明》9.3 节 `wf.*` 事件）。
...
- **引擎不持久化**：SpiffWorkflow 本身不存流程状态，流程实例/任务/审批记录必须由本项目自建表维护，靠序列化/反序列化跨请求恢复，别指望引擎自带存储。
...
| 文档 | 说明 |
| --- | --- |
| 平台《项目规划说明》3.1 节 | 选型说明：SpiffWorkflow 条目与 Python 3.14 兼容口径 |
| 平台《项目规划说明》5 节 | 功能模块：审批（流程建模、流程实例、我的待办/已办） |
| 平台《项目规划说明》9.3 节 | 事件总线：`wf.*` 流程事件 |
