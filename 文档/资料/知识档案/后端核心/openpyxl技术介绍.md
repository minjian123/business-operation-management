...
它是 Python 生态处理 xlsx 的事实标准，在本项目中支撑
**用户批量导入**与**列表导出**两类通用能力。

- **定位**：本项目  Excel 读写（导入 / 导出）。
...
- 支撑**用户批量导入**：解析上传的 xlsx，逐行校验后落库（见平台《架构设计 · 总体架构》6.1 节、功能模块「导入导出」）。
- 支撑**列表导出**：把查询结果写为 xlsx 返回下载（见平台《项目规划说明》5 节）。
- 导入为写操作，支持**幂等键**（请求头 `Idempotency-Key`）防重复提交（见平台《项目规划说明》8 节 API 设计规范）。
- 导出涉及敏感字段时做**脱敏**（手机号/邮箱掩码），持 `data:plain` 权限码才出明文（见平台《项目规划说明》12 节数据脱敏）。
- openpyxl 为同步库、阻塞 I/O，导入/导出应放入**线程池或 Celery 任务**执行，避免卡住事件循环（见《[FastAPI 技术介绍](FastAPI技术介绍.md)》注意事项）。
- 导入模板、导出文件经 [python-multipart](python-multipart技术介绍.md) 上传、产物可存 [MinIO](MinIO技术介绍.md)（见平台《架构设计 · 总体架构》6.1 节文件管理）。
- 验收口径：「Excel 导入导出可用」纳入功能测试与 MVP 验收（见平台《项目规划说明》16 节）。
...
| 文档 | 说明 |
| --- | --- |
| 平台《架构设计 · 总体架构》6.1 节 | 后端技术栈：Excel 处理（openpyxl）条目 |
| 平台《项目规划说明》5 节 | 功能模块：导入导出（用户批量导入、列表导出） |
| 平台《项目规划说明》8 节 | API 设计规范：导入幂等键约定 |
| 平台《项目规划说明》12 节 | 认证与安全：导出脱敏、data:plain 权限 |
