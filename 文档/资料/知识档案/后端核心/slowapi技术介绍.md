...
在本项目中它承担接口级限流，重点是**登录防爆破**。

- **定位**：本项目接口级限流方案，登录防爆破优先。
...
- 作为 FastAPI 限流方案，用装饰器声明限流规则，支持 IP / 用户 / client 三种维度（见平台《架构设计 · 总体架构》6.1 节）。
- 计数走 [Redis](Redis技术介绍.md) 后端，多实例集群共享同一计数，避免「每实例各限各的」导致总配额被放大。
- Redis 故障时降级为 memory 后端，限流能力不中断（见平台《项目规划说明》14 节「限流与降级」）。
- 登录防爆破优先：如登录接口「5 次锁 15 分钟」，与 [Pillow](Pillow技术介绍.md) 图形验证码叠加（见平台《项目规划说明》12 节「认证与安全」）。
- 开放接口（`/api/open`）按 client 维度独立限流（见平台《项目规划说明》9.4 节）。
- AI 接口独立限流与预算控制（见平台《项目规划说明》12 节 AI 安全）。
...
| 文档 | 说明 |
| --- | --- |
| 平台《架构设计 · 总体架构》6.1 节 | 后端技术栈与选型说明（slowapi 条目） |
| 平台《项目规划说明》12 节 | 认证与安全：登录限流、AI 接口独立限流 |
| 平台《项目规划说明》14 节 | 性能与高并发设计：分布式限流与降级 |
