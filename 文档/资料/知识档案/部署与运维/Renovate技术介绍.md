...
- **定位**：本项目依赖自动升级，自动提 MR 升级依赖（平台《架构设计 · 总体架构》6.4 节）。
- **版本**：3x.x 系列（持续快速迭代）。
- **许可**：AGPL-3.0。本项目自托管运行、不修改源码，无传染影响（平台《项目规划说明》2.5 节）。
- **落地形态**：阶段二起以容器方式随 gitlab.yml 编排在 mjbk 运行，需 mjbk 可访问外网（平台《开发部署规划》4.5 节）。
...
| automerge | CI 通过后自动合入；本项目建议保持人工评审，仅对低风险依赖开放 |
| labels | 自动给 MR 打标签（renovate / 依赖类型 / 安全更新），便于过滤与统计 |
| platform | 目标平台：GitHub、GitLab（含自托管）等，本项目用自托管 GitLab |
...
- **依赖自动升级**：Python（pyproject / uv.lock）、前端（package.json / lockfile）、容器镜像标签统一由 Renovate 提 MR，CI 验证后人工合入（平台《项目规划说明》3.4 节）。
- **安全补丁通道**：与 GitLab Dependency Scanning 互补——扫描发现漏洞、Renovate 提安全更新 MR（平台《项目规划说明》16 节安全专项）。
- **运行方式**：阶段二起以容器方式随 gitlab.yml 编排，常驻 mjbk；需 mjbk 可访问外网（拉取版本发布信息），不可达时镜像同步与依赖升级暂缓（平台《开发部署规划》4.5 节、11 节）。
...
| 候选技术 | 优缺点 | 结论 |
| --- | --- | --- |
| **Renovate（选中）** | 优点：事实标准、原生支持自托管 GitLab、manager 覆盖全、规则灵活；缺点：需自己跑服务（本项目已容器化，成本可忽略） | 自托管 GitLab 场景下依赖升级的最优解 |
| Dependabot | 优点：GitHub 内置零配置；缺点：只服务 GitHub，不支持自托管 GitLab | 本项目主仓库在自托管 GitLab，不适用 |
...
- **AGPL 边界**：作为独立服务运行、不改源码、不深度定制，遵守平台《项目规划说明》2.5 节约束。
- **外网依赖**：Renovate 要访问版本发布源与 GitLab API，mjbk 外网不可达时升级暂停，属预期行为（平台《开发部署规划》11 节）。
- **MR 刷屏**：默认每个包一个 MR，包多的仓库会刷屏——用 grouping + schedule 收敛。
- **automerge 风险**：自动合入绕过人工评审，本项目默认关闭，仅对低风险依赖（如 dev 工具链）谨慎开放。
...
| 文档 | 说明 |
| --- | --- |
| 平台《架构设计 · 总体架构》6.4 节 | 部署与运维技术栈（依赖升级条目） |
| 平台《项目规划说明》3.4 节 | 选型说明：原生支持自托管 GitLab、自动提 MR |
| 平台《开发部署规划》4.5 节 | mjbk 运行形态：随 gitlab.yml 编排、外网依赖 |
