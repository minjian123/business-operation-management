...
- **定位**：本项目阶段十五 AI 能力的配套组件——Milvus 向量库的元数据与集群协调依赖（平台《架构设计 · 总体架构》6.4 节）。
- **版本**：3.5 / 3.6 系列（3.5 为长期维护线，3.6 为新版）。
- **许可**：Apache-2.0，免费开源（平台《项目规划说明》2.5 节）。
- **落地形态**：随 Milvus 于阶段十五加入 Compose 编排，MinIO 复用（平台《开发部署规划》4.3 节）。
...
- **Milvus 依赖**：Milvus 用 etcd 存元数据（collection / 分区 / 索引信息）并做集群协调（节点发现、选主），是 Milvus 的标准组件（平台《架构设计 · 总体架构》6.4 节「Milvus（milvus + etcd，MinIO 复用）」）。
- **阶段十五随 AI 落地**：etcd 与 Milvus 一并加入 Compose 编排，MinIO 对象存储复用现有实例（平台《开发部署规划》4.3 节）。
- **数据落盘**：etcd 对磁盘 fsync 延迟敏感，数据放 NVMe SSD 命名卷（平台《开发部署规划》4.2 节）。
- **边界说明**：本项目业务代码不直接依赖 etcd，它只服务 Milvus 内部协调，运维上「了解即可」（《[技术栈知识档案总览](../技术栈知识档案总览.md#path)》学习路径）。
...
| Consul | 优点：KV + 服务发现 + 健康检查一体、多数据中心强；缺点：组件面宽、本项目只用 KV 属功能过剩 | 需要服务发现时再考虑 |
...
- **别当缓存用**：etcd 为协调 / 元数据设计，高频读写大对象会拖垮集群；本项目缓存走 Redis。
...
- **备份**：etcd 数据 = Milvus 的「户籍档案」，纳入 mjbk 每日备份范围（平台《开发部署规划》8 节）。
