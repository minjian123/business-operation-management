...
在本项目里它承担统一入口：托管前端静态资源、反向代理 API、TLS 终止与负载均衡。

- **定位**：本项目生产入口层，所有浏览器流量先到 nginx 再分发（平台《架构设计 · 总体架构》6.4 节）。
- **版本**：1.2x 系列（stable 与 mainline 双轨发布，生产用 stable）。
- **许可**：BSD-2-Clause，免费开源无商用限制（平台《项目规划说明》2.5 节）。
- **落地形态**：frontend 容器内运行，托管 PC + 移动端静态资源并反代 /api（平台《项目规划说明》3.4 节）。
...
- **静态托管**：托管前端构建产物（PC 管理端 + 移动端），浏览器直接拿静态资源，不打扰后端（平台《项目规划说明》3.4 节）。
- **反向代理**：`/api` 反代到后端多副本；生产同源部署，不开放跨域（平台《项目规划说明》17 节）。
- **负载均衡 + TLS 终止**：后端无状态可横向扩展，nginx 负责分发与 HTTPS 解密（平台《项目规划说明》19.1 节）。
- **子域名租户路由**：多租户按子域名区分入口，配泛域名证书（平台《架构设计 · 总体架构》6.4 节）。
- **WebSocket 支持**：`/socket.io` 路径放行 Upgrade/Connection 头，负载均衡用 ip_hash / Cookie 亲和，保证 Socket.IO 连接升级后固定在同一后端实例（平台《项目规划说明》19.2 节）。
- **安全响应头**：统一配置 CSP、HSTS（dev 关闭）、X-Frame-Options（SAMEORIGIN，兼容同源 Grafana 嵌入）、X-Content-Type-Options、Referrer-Policy、Permissions-Policy（平台《项目规划说明》19.2 节）。
- **文档收敛**：生产环境关闭 Swagger/ReDoc 在线文档，对外只发 CI 导出的 swagger.json 快照（平台《项目规划说明》8 节）。
...
| HAProxy | 优点：L4/L7 负载均衡专家，性能极强；缺点：静态托管弱，功能面窄 | 纯 LB 场景可用，本项目需要静态托管，不单独选它 |
| OpenResty | 优点：nginx + Lua 脚本，动态逻辑强；缺点：多一层 Lua 学习成本，本项目用不到 | 过度设计，不采用 |
...
| 文档 | 说明 |
| --- | --- |
| 平台《架构设计 · 总体架构》6.4 节 | 部署与运维技术栈（nginx 条目） |
| 平台《项目规划说明》3.4 节 | 选型说明：静态托管 / 反代 / 负载均衡 / 子域名租户路由 |
| 平台《项目规划说明》19.1 节 | 部署拓扑：nginx 为统一入口 |
| 平台《项目规划说明》19.2 节 | WebSocket 放行、安全响应头、X-Frame-Options 约定 |
